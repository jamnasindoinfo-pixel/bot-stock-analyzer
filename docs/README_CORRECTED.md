...
├── config/                     # Konfigurasi
├── .claude/                    # Aturan global Claude
│   └── rules.md               # Rules wajib untuk Claude
...
└── cleanup_helper.py          # Helper script untuk cleanup
```
...
## 🤖 Aturan Global Claude (IMPORTANT!)

Claude memiliki aturan wajib yang harus selalu diikuti:

### 1. Auto Cleanup
- **HAPUS** semua file sementara setelah test selesai
- File yang dihapus: `debug_*.py`, `test_*.py`, `investigate_*.py`, `*.tmp`, `*.log`
- Claude akan menggunakan `cleanup_helper.py` untuk otomasi

### 2. Folder Organization
- **Rapikan** struktur folder setelah selesai
- File harus berada di folder yang sesuai
- Claude akan memindahkan file otomatis jika perlu

### 3. Implementation
```bash
# Jalankan cleanup (Claude akan otomatis menjalankan ini)
python cleanup_helper.py
```

Lihat `.claude/rules.md` untuk aturan lengkap.
